...
/**
 * @file   MonoDataProvider.h
 * @brief  Parse Mono-VIO  dataset.
 ...
 */
class MonoDataProvider : public DataProviderInterface {
 public:
  KIMERA_DELETE_COPY_CONSTRUCTORS(MonoDataProvider);
  KIMERA_POINTER_TYPEDEFS(MonoDataProvider);
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  //! Ctor with params.
  MonoDataProvider(const std::string& dataset_path,
                    const int& initial_k,
                    const int& final_k,
                    const VioParams& vio_params);

  //! Ctor from gflags
  explicit MonoDataProvider(const VioParams& vio_params);

  virtual ~MonoDataProvider();
