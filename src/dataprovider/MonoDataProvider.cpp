...
/**
 * @file   MonoDataProvider.cpp
 * @brief  Parse EUROC dataset.
 * @author Prasad Antervedi
 * @author Antoni Rosinol
 * @author Yun Chang
 * @author Luca Carlone
 */

#include "kimera-vio/dataprovider/MonoDataProvider.h"
...
/* -------------------------------------------------------------------------- */
MonoDataProvider::MonoDataProvider(const std::string& dataset_path,
                                     const int& initial_k,
                                     const int& final_k,
  ...
  if (!shutdown_ && !dataset_parsed_) {
    LOG(INFO) << "Parsing MonoEuroc dataset...";
    parse();
...
/* -------------------------------------------------------------------------- */
MonoDataProvider::MonoDataProvider(const VioParams& vio_params)
    : MonoDataProvider(FLAGS_dataset_path,
                        FLAGS_initial_k,
                        FLAGS_final_k,
                        vio_params) {}

/* -------------------------------------------------------------------------- */
MonoDataProvider::~MonoDataProvider() {
  LOG(INFO) << "ETHDatasetParser destructor called.";
}

/* -------------------------------------------------------------------------- */
bool MonoDataProvider::spin() {
  if (dataset_parsed_) {
...
/* -------------------------------------------------------------------------- */
bool MonoDataProvider::spinOnce() {
  ...
  if (available_left_img ) {
    ...
    LOG(ERROR) << "Missing left frame, proceeding to the next one.";
  }
...
void MonoDataProvider::sendImuData() const {
  ...
  for (const ImuMeasurement& imu_meas : imu_measurements_) {
    CHECK_GT(imu_meas.timestamp_, previous_timestamp)
        << "Mono IMU data is not in chronological order!";
...
/* -------------------------------------------------------------------------- */
bool MonoDataProvider::parseImuData(const std::string& input_dataset_path,
...
/* -------------------------------------------------------------------------- */
bool MonoDataProvider::parseGtData(const std::string& input_dataset_path,
...
/* -------------------------------------------------------------------------- */
bool MonoDataProvider::parseDataset() {
  // Parse IMU data.
...
/* -------------------------------------------------------------------------- */
bool MonoDataProvider::parseCameraData(const std::string& cam_name,
...
std::string MonoDataProvider::getDatasetName() {
...
/* -------------------------------------------------------------------------- */
gtsam::Pose3 MonoDataProvider::getGroundTruthRelativePose(
...
/* -------------------------------------------------------------------------- */
bool MonoDataProvider::isGroundTruthAvailable(
    const Timestamp& timestamp) const {
  return timestamp > gt_data_.map_to_gt_.begin()->first;
}

/* -------------------------------------------------------------------------- */
bool MonoDataProvider::isGroundTruthAvailable() const {
  return is_gt_available_;
}

/* -------------------------------------------------------------------------- */
VioNavState MonoDataProvider::getGroundTruthState(
    const Timestamp& timestamp) const {
  const auto& it_low =
...
const InitializationPerformance MonoDataProvider::getInitializationPerformance(
...
/* -------------------------------------------------------------------------- */
size_t MonoDataProvider::getNumImages() const {
...
/* -------------------------------------------------------------------------- */
size_t MonoDataProvider::getNumImagesForCamera(
...
/* -------------------------------------------------------------------------- */
bool MonoDataProvider::getImgName(const std::string& camera_name,
                                   const size_t& k,
...
/* -------------------------------------------------------------------------- */
Timestamp MonoDataProvider::timestampAtFrame(const FrameId& frame_number) {
...
void MonoDataProvider::clipFinalFrame() {
...
/* -------------------------------------------------------------------------- */
void MonoDataProvider::print() const {
