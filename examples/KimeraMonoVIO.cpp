//
// Created by Prasad Antervedi on 1/18/22.
//
/* ----------------------------------------------------------------------------
...
int main(int argc, char* argv[]) {
    ...
    case 0: {
      switch (vio_params.frontend_type_) {
        case VIO::FrontendType::kMonoImu: {
          dataset_parser =
              VIO::make_unique<VIO::MonoDataProvider>(vio_params);
        } break;
        default: {
          LOG(FATAL) << "Unrecognized Frontend type: or Stereo used  "
  ...
  switch (vio_params.frontend_type_) {
    ...
    case VIO::FrontendType::kStereoImu: {
      std::cout<<"This executable is for Monocular VIO\n";
  ...
  /*if (vio_params.frontend_type_ == VIO::FrontendType::kStereoImu) {
    VIO::StereoImuPipeline::Ptr stereo_pipeline =
        VIO::safeCast<VIO::Pipeline, VIO::StereoImuPipeline>(
            vio_pipeline);*/
  if (vio_params.frontend_type_ == VIO::FrontendType::kMonoImu) {
    VIO::MonoImuPipeline::Ptr monoo_pipeline =
        VIO::safeCast<VIO::Pipeline, VIO::MonoImuPipeline>(
            vio_pipeline);

    /*dataset_parser->registerRightFrameCallback(
        std::bind(&VIO::MonoImuPipeline::fillRightFrameQueue,
                  monoo_pipeline,
                  std::placeholders::_1));*/
  }
